...
module('Integration | Component | gif/detailed', function (hooks) {
  ...
  test('it renders a header with a share button', async function (assert) {
    await render(hbs`<Gif::Detailed @gif={{this.gif}} />`);

    assert.dom('.jumbo').exists();
    assert.dom('.jumbo h2').containsText('You Got It Art GIF by GIPHY Studios Originals');
    assert.dom('.jumbo a.button').containsText('Share on Twitter');
  });

  test('it renders detailed information about a gif property', async function (assert) {
    await render(hbs`<Gif::Detailed @gif={{this.gif}} />`);

    assert.dom('article').hasClass('gif');
    assert.dom('article h3').containsText('About You Got It Art GIF by GIPHY Studios Originals');
    assert.dom('article .detail.owner').containsText('Creator: N/A');
    assert.dom('article .detail.type').containsText('GIPHY Profile: https://giphy.com/studiosoriginals/');
    assert.dom('article .detail.location').containsText('Description: N/A');
    assert.dom('article .detail.bedrooms').containsText('GIPHY Link: https://giphy.com/gifs/studiosoriginals-2m58Jife4Uz0TSpAS4');
